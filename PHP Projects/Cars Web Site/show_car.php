...
echo "<tr valign='top'>";
echo "<td width='250'> марка: <b>".htmlspecialchars(stripcslashes($row['make'])). "</b><br>
цена: <b>".htmlspecialchars(stripcslashes($row['price']))."</b> лв.<br><hr>
<span style='font-size:16px'><pre>".htmlspecialchars(stripcslashes($row['moreinfo']))."</pre></span></td>
<td>".($row['picture']?"<img src='pictures/".$row['picture']."'>":"Няма снимка...")."</td>";
echo "</tr>";
echo "</table>";
echo "<a href='javascript:history.back()' title='Връщане към предходната страница'>
Обратно към списъка</a>";
$mysqli->close();
?>
</div>
